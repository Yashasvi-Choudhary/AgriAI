...
function updateNutrientAnalysis(n, p, k) {
  ...
  document.getElementById("nutrientAnalysis").innerHTML =
    analysis
      .map(
        (item) => `
      <div class="rounded-2xl border border-backgroundDark p-4 text-center">
        <p class="text-sm font-semibold text-textDark">${item.name}</p>
        <p class="mt-2 text-3xl font-bold text-textDark">${item.value}</p>
        <p class="mt-1 text-xs text-textLight">${item.label}</p>
      </div>
    `
      )
      .join("");
...
function renderResult(data) {
  ...
  recommendationsList.innerHTML = `
    <div class="rounded-2xl border border-backgroundDark p-5 bg-slate-50">
      <p class="text-2xl font-semibold text-textDark">
        ${rec.fertilizer_name}
      </p>
      <p class="mt-2 text-sm text-textMid">
        ${rec.recommended_quantity}
      </p>
      <p class="mt-4 text-sm text-textDark">
        ${rec.reason}
      </p>
      <p class="mt-3 text-sm text-textLight">
  ...
  document.getElementById("scheduleList").innerHTML =
    scheduleItems[pageLang]
      .map(
        (item) => `
      <div class="rounded-2xl bg-slate-50 border border-backgroundDark p-4">
        <p class="text-sm font-semibold text-textDark">${item.title}</p>
        <p class="mt-1 text-xs text-textLight">${item.detail}</p>
      </div>
    `
      )
      .join("");
  ...
  document.getElementById("tipsList").innerHTML =
    tipsItems[pageLang]
      .map(
        (tip) =>
          `<li class="text-sm text-textLight">• ${tip}</li>`
      )
      .join("");
  ...
  fetch("/predict", {
    ...
    .then(async (res) => {
      ...
      if (!res.ok || !body || body.status !== "success") {
        throw new Error(
          body?.message || messages.apiError[pageLang]
        );
      }
...
// expose globally
window.getFertilizerRecommendation =
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (fertilizerBtn) {
    fertilizerBtn.addEventListener(
      "click",
      getFertilizerRecommendation
    );
  }
